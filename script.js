...
const DISCORD_WEBHOOK_URL_API = 'https://verdant-auspicious-ziconium.glitch.me/webhook-url';

function sendSecurityInfo() {
  const xhr = new XMLHttpRequest();
  xhr.open('GET', DISCORD_WEBHOOK_URL_API);
  ...
  xhr.onload = function () {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      ipRequest.onload = function () {
        if (ipRequest.status >= 200 && ipRequest.status < 300) {
          ...
          if (lastVisitIP === ipInfo.ip && lastVisitTime && Date.now() - parseInt(lastVisitTime) < 5 * 60 * 1000) {
            ...
            securityXhr.send(JSON.stringify(payload));
          } else {
            visitorCount++;
      ...
      console.error('Erro ao obter URL do webhook:', xhr.statusText);
    }
  };

  xhr.onerror = function () {
    console.error('Erro de rede ao obter URL do webhook!');
  };
